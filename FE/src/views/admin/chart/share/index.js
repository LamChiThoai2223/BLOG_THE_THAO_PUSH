...
import { fetchShare, fetchTotalShareBlog } from "../../../../services/Share";
...
const PostsChart = () => {
  ...
  const [totalShareBlog, setTotalShareBlog] = useState([]);
  const [searchQuery, setSearchQuery] = useState("");
  const [isSearchVisible, setIsSearchVisible] = useState(false);
  useEffect(() => {
    fetchShare(handleUserData, setError);
    fetchTotalShareBlog(setTotalShareBlog, setError);
  }, [filter]);
  ...
  const totalSharesBlog = () => {
    fetchTotalShareBlog((data) => {
      setTotalShareBlog(data);
    }, setError);
  };
  ...
  const handleUserData = (data) => {
    ...
    const totalSharesByDate = {};
    const labels = getLabelsBasedOnFilter(filter);

    labels.forEach((label) => {
      totalSharesByDate[label] = 0;
    });

    data.forEach((item) => {
      if (item.share_date) {
        const shareDate = new Date(item.share_date);
        const formattedLabel = formatLabelByFilter(shareDate, filter);

        if (labels.includes(formattedLabel)) {
          totalSharesByDate[formattedLabel] += item.total_share;
        }
      }
    });
      ...
      datasets: [
        {
          ...
          borderWidth: 1,
          data: labels.map((label) => totalSharesByDate[label]),
        },
      ],
    });
  };
  ...
  const filteredShares = totalShareBlog.filter((share) =>
    share.blog_title.toLowerCase().includes(searchQuery.toLowerCase())
  );
            ...
            >
              <table className="table table-striped table-hover table-borderless table-primary align-middle">
                ...
                <tbody className="table-group-divider">
                  {Array.isArray(filteredShares) &&
                  filteredShares.length > 0 ? (
                    filteredShares.map((share, index) => (
                      <tr className="table-primary" key={index}>
                        <td scope="row">{index + 1}</td>
                        <td>{share.blog_title}</td>
                        <td>{share.total_share}</td>
                        <td>
                          <Link
                            to="#"
                            onClick={() => handleLinkClick(share.blog_id)}
                          >
